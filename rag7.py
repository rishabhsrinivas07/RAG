    ...
    for pdf_path in pdf_files:
        ...
        for i, page in enumerate(reader.pages):
            ...
            if force_ocr:
                try:
                    ...
                    # ✅ Only use VLM output if it actually produced content
                    if vlm_text and len(vlm_text.strip()) > 20:
                        text = vlm_text
                        extraction_method = "vlm_ocr"
                        ocr_count += 1
                    else:
                        print(f"   ⚠️ VLM returned empty for {pdf_path.name} page {i+1}, keeping standard text")
                except Exception as e:
                    print(f"⚠️ VLM extraction failed for {pdf_path.name} page {i+1}: {e}")
                    # ✅ Keep original text instead of setting text = ""
        ...
        if pages:
            ...
            with open("ingestion_log.txt", "w") as f:
                for c in all_chunks:
                    f.write(f"[{c.metadata.get('source')} | Page {c.metadata.get('page')}]\n")
                    f.write(c.page_content + "\n---\n")
                    print(f"✅ Full ingestion log written to ingestion_log.txt")
