...
def vlm_extract_text(image_source, source_name="image", max_retries=2):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            print(f"⚠️ VLM attempt {attempt+1} failed: {e}")
...
def ingest_texts(texts: list[str]):
    ...
    print(f"✅ Ingested {len(chunks)} text chunks")
...
def ingest_folder(root_folder: str, ocr_threshold: int = 50):
    ...
    # ✅ Add a timestamp header so you know when this specific run started
    with open(LOG_FILE_PATH, "a", encoding="utf-8") as f:
        f.write(f"\n{'#'*80}\n")
    ...
    for pdf_path in pdf_files:
        ...
        if pages:
            ...
            print(f"   📄 {pdf_path.name}: {len(chunks)} chunks ({ocr_count} via VLM)")

    if all_chunks:
        vectorstore.add_documents(all_chunks)
        print(f"✅ Total ingested: {len(all_chunks)} chunks")
        print(f"📝 Logs appended to: {LOG_FILE_PATH.resolve()}")
...
def ingest_images(image_paths: list[str]):
    ...
    if all_chunks:
        vectorstore.add_documents(all_chunks)
        print(f"✅ Ingested {len(all_chunks)} image chunks")
...
def ingest_urls(urls: list[str], delay: float = 2.0):
    ...
    for url in urls:
        try:
            ...
            docs = loader.load()
            for doc in docs: doc.metadata["source"] = url
    ...
    if all_chunks:
        vectorstore.add_documents(all_chunks)
        print(f"✅ Ingested {len(all_chunks)} web chunks")